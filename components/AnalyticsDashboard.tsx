...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-900 px-3 py-2 shadow-lg text-xs">
...
function StatCard({
  ...
  return (
    <div className={`p-5 rounded-xl bg-white dark:bg-zinc-800/60 border-2 ${borderClass} transition-all duration-300 shadow-sm hover:shadow-md`}>
      <div className="mb-3">
        <Icon className={`h-6 w-6 ${colorClass}`} />
      </div>
      <div className={`text-3xl font-black ${colorClass}`}>{value}</div>
      <div className="text-sm text-zinc-700 dark:text-zinc-300 mt-1 font-semibold">{label}</div>
...
function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`p-5 rounded-xl bg-white dark:bg-zinc-800/50 border-2 border-zinc-200 dark:border-zinc-700 shadow-sm ${className}`}>
      {children}
    </div>
  );
}
...
export default function AnalyticsDashboard({ reviews, repos }: Props) {
  ...
  /* Activity timeline — group reviews by date (last 30 days) */
  const activityData = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      byDate.set(d.toISOString().slice(0, 10), { reviews: 0, additions: 0, deletions: 0 });
    }
    reviews.forEach(r => {
  ...
  if (reviews.length === 0 && repos.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-zinc-900 dark:text-zinc-100 font-semibold text-lg">No analytics data yet</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex items-center gap-4 p-4 rounded-xl bg-white dark:bg-zinc-800/50 border-2 border-green-300 dark:border-green-800 shadow-sm">
          ...
          <div>
            ...
            <div className="text-sm text-green-800 dark:text-green-400 font-semibold">Lines added</div>
          </div>
        </div>
        <div className="flex items-center gap-4 p-4 rounded-xl bg-white dark:bg-zinc-800/50 border-2 border-red-300 dark:border-red-800 shadow-sm">
          <div className="p-2.5 rounded-lg bg-red-100 dark:bg-red-900/50">
            <Minus className="h-5 w-5 text-red-700 dark:text-red-400" />
          </div>
          <div>
            <div className="text-2xl font-black text-red-700 dark:text-red-300">
              -{stats.totalDeletions.toLocaleString()}
            </div>
            <div className="text-sm text-red-800 dark:text-red-400 font-semibold">Lines removed</div>
          </div>
        </div>
      </div>
      ...
      {activityData.length > 0 && (
        <Panel>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Review Activity (Last 30 Days)</h4>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {repoDistribution.length > 0 && (
          <Panel>
            <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-4">Reviews by Repository</h4>
            <div className="w-full h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={repoDistribution} layout="vertical" margin={{ left: 0, right: 16 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(113,113,122,0.15)" horizontal={false} />
                  <XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: '#71717a' }} tickLine={false} axisLine={false} />
                  <YAxis
                    type="category"
                    dataKey="name"
        ...
        {codeChurnByRepo.length > 0 && (
          <Panel>
            <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-4">Code Churn by Repository</h4>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Legend
                    iconType="circle"
                    iconSize={8}
                    formatter={(value: string) => <span className="text-xs text-zinc-600 dark:text-zinc-400 font-medium">{value}</span>}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
        ...
        {topContributors.length > 0 && (
          <Panel>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Top Contributors</h4>
            </div>
              ...
              {topContributors.map((c, i) => {
                ...
                return (
                  <div key={c.author} className="flex items-center gap-3">
                    <span className="text-xs font-bold text-zinc-500 dark:text-zinc-400 w-5 text-right">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 truncate">{c.author}</span>
        ...
        {recentReviews.length > 0 && (
          <Panel>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100">Recent Reviews</h4>
            </div>
            <div className="space-y-1">
              {recentReviews.map((r) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 truncate group-hover:text-primary transition-colors">
                  ...
                  {(r.additions || r.deletions) ? (
                    <div className="text-xs font-mono whitespace-nowrap font-bold">
                      <span className="text-green-600 dark:text-green-400">+{r.additions || 0}</span>{" "}
                      <span className="text-red-600 dark:text-red-400">-{r.deletions || 0}</span>
                    </div>
                  ) : null}
                </a>
              ))}
